...
class smartCurriculumCallback(BaseCallback):
    ...
    """
    def __init__(self, envFunct, refreshRate, betaRate, initCurriculum, endDomain, targetDomain, domainPowers, ADRMethod = 'loss',targetReliability=None, targetReward=None, renders = True, verbose=1):
...
class PPOtraining():
    def __init__(self, envFunct, trainingSteps, targetDomain, domainPowers, domainProgressRate, learningRate = 0.0003, batchSize = 256, ADRMethod ='loss', autoParameters = False, startDomain=None, endDomain=None, targetReliability=None, targetReward=None, initModelLoc=None, render=False, verbose = 1, tag="", seed=1)  :
        ...
        KwArgs :
            ADRMethod : str. Default is 'loss'. Refers to the chosen ADR function used in the callback function. Note that only 'loss' is considered to work for now 
            ...
            tag : str. default is "". Puts a WandB label on the best saved network 
            seed: int. default is 0. Sets the seed for the model's initial weights.
        """
        self.seed = seed
    ...
    def updateModel(self):
        if self.modelLoc==None: 
            self.model = PPO(MlpPolicy, self.env, tensorboard_log="./logger/",verbose=1, device='cuda',n_steps = 2048, n_epochs=10, batch_size= self.batchSize, learning_rate= self.learningRate, seed = self.seed)
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        tag = params["tag"]
        wandb.init(project="Smart_Quad_Friction_Seed", sync_tensorboard=True, allow_val_change=True, reinit=True, tags=[tag])
