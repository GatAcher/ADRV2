...
if __name__ == '__main__':
    wandb.init(project="Smart_Bambi", sync_tensorboard=True, allow_val_change=True, reinit=True)
    
    wandb.config.progress_rate = 0.2
    wandb.config.domain_powers = None
    wandb.config.learningRate = 0.002
    
    wandb.config.batchSize = 1800
    #wandb.config.update({'learningRate':0.0005, 'batchSize':1740}, allow_val_change=True)
    training = PPOtraining(quad_env.QuadEnv, 2000000, np.array([0.45,0.65]), np.array([0.000001,0.000001]), 0.02, learningRate = wandb.config.learningRate, batchSize = wandb.config.batchSize, startDomain=np.array([0.45,0.45]), endDomain = np.array([0.45,0.65]), ADRMethod = 'loss', targetReliability=None, targetReward=None, initModelLoc=None, render = False, verbose = 1)
    #[0.10,0.10,0.2]
    #[0.02,0.05,0.2]
    #Implementer rewardScale proprement
    #Implementer saveBestNetwork dans le callback
    #experiment with batch size (adaptive ?) (also adaptive learning rate)

    #Common HyperParameters such as Batch_size, N_epoch, have a huge impact on the results. especially Batch_size with DR
    # https://docs.ray.io/en/master/rllib-algorithms.html?highlight=ppo#proximal-policy-optimization-ppo   implementation
    # Batch size parameter sweep
    # endDomain sweep
    # disable action noise
    # set up a meeting with kez